 ...
 */
	...
	class SimpleDiscogsPlugin extends BaseApplicationPlugin {
		# -------------------------------------------------------
		protected $description = 'Simple Discogs Import plugin for Collectiveaccess';
		# -------------------------------------------------------
		private $opo_config;
		...
		public function __construct($ps_plugin_path) {
			...
			$this->description = _t('Simple Discogs import');
			parent::__construct();
			$conf_file = $ps_plugin_path.'/conf/SimpleDiscogs.conf';
		...
		public function hookRenderMenuBar($pa_menu_bar) {
			if ($o_req = $this->getRequest()) {
				...
				$va_menu_items["z3950"] = array(
					'displayName' => "Discogs",
					"default" => [
						'module' => 'SimpleDiscogs',
						'controller' => 'SimpleDiscogs',
						'action' => 'Index'
					]
				);
				if (isset($pa_menu_bar['Import'])) {
		...
		static function getRoleActionList() {
			return array(
				'can_use_simple_discogs_plugin' => array(
						'label' => _t('Can use SimpleDiscogs plugin'),
						'description' => _t('Can use SimpleDiscogs plugin')
					)
			);
		}
